      ...
      <v-card>
        <v-form fast-fail @submit.prevent="login">
          <v-card-text>
            ...
            <v-btn @click="this.$router.push('/mot-de-passe-oublie')" color="secondary" block size="default"
              variant="flat" prepend-icon="mdi-question-mark" class="mb-4">
...
export default {
  ...
  beforeMount() {
    console.log(this.$store.getters['sessionStore/isAuthenticated']);
    if (this.$store.getters['sessionStore/isAuthenticated']) {
      this.$router.push('/');
    }
  },
}
